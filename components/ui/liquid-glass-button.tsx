...
const liquidbuttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full text-sm font-semibold outline-none transition-[color,transform,box-shadow] duration-300 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-transparent text-white hover:scale-[1.03]",
...
export function LiquidButton({
  ...
  const content = (
    <>
      ...
      />
      <div
        className="absolute inset-0 -z-10 overflow-hidden rounded-full border border-white/15 bg-white/10"
        style={{ backdropFilter: 'url("#container-glass")' }}
      />
      <div className="absolute inset-[1px] rounded-full bg-[linear-gradient(180deg,rgba(255,255,255,0.2),rgba(255,255,255,0.04))]" />
  ...
  if (asChild && React.isValidElement(children)) {
    ...
    return React.cloneElement(child, {
      ...props,
      className: cn("relative isolate", liquidbuttonVariants({ variant, size, className }), child.props.className),
      children: content
    });
  }
