      ...
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        <aside className="admin-surface rounded-3xl p-4 sm:p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => setProjectForEditing(null)}
              className="rounded-full border border-cyan-200/50 bg-cyan-200/15 px-3 py-1 text-xs font-semibold text-cyan-50"
            >
              Nytt
            </button>
          </div>
              ...
              <div
                ...
                className={cn(
                  ...
                  selectedId === project.id
                    ? "border-cyan-200/60 bg-cyan-200/16"
                    : "border-white/15 bg-white/6 hover:border-white/35"
                )}
              >
                <button
                  type="button"
                  onClick={() => setProjectForEditing(project.id)}
                  className="w-full text-left"
                >
                  <p className="text-sm font-semibold text-white">{project.title}</p>
                  <p className="mt-1 text-xs text-white/60">/{project.slug}</p>
                </button>
                <button
                  type="button"
                  onClick={() => handleDelete(project.id)}
                  className="mt-2 rounded-full border border-rose-300/40 bg-rose-300/10 px-2.5 py-1 text-[11px] font-semibold text-rose-100"
                >
                  Radera
                </button>
              </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-5">
            <div className="grid gap-4 md:grid-cols-2">
              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Titel</span>
                <input
                  {...register("title")}
                ...
                />
                {errors.title ? <p className="mt-1 text-xs text-rose-100">{errors.title.message}</p> : null}
              </label>

              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Slug</span>
                <input
                  {...register("slug")}
                  className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
                />
                {errors.slug ? <p className="mt-1 text-xs text-rose-100">{errors.slug.message}</p> : null}
              </label>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Kategori</span>
                <select
                  {...register("category")}
                  className="w-full rounded-xl border border-white/20 bg-slate-900/85 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
                >
              ...
              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Status</span>
                <select
                  {...register("status")}
                  className="w-full rounded-xl border border-white/20 bg-slate-900/85 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
                >
            ...
            <label className="block">
              <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Kort beskrivning</span>
              <textarea
                rows={3}
                {...register("shortDescription")}
                className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
              />
              {errors.shortDescription ? <p className="mt-1 text-xs text-rose-100">{errors.shortDescription.message}</p> : null}
            </label>

            <label className="block">
              <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Lång beskrivning</span>
              <textarea
                rows={6}
                {...register("longDescription")}
                className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
              />
              {errors.longDescription ? <p className="mt-1 text-xs text-rose-100">{errors.longDescription.message}</p> : null}
            </label>

            <div className="grid gap-4 md:grid-cols-2">
              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Tags</span>
                <input
                  {...register("tagsInput")}
              ...
              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Tech stack</span>
                <input
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Demo URL</span>
                <input
                  {...register("demoUrl")}
                  placeholder="https://"
                  className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
                />
                {errors.demoUrl ? <p className="mt-1 text-xs text-rose-100">{errors.demoUrl.message}</p> : null}
              </label>

              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Repo URL</span>
                <input
                  {...register("repoUrl")}
                  placeholder="https://"
                  className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
                />
                {errors.repoUrl ? <p className="mt-1 text-xs text-rose-100">{errors.repoUrl.message}</p> : null}
              </label>

              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Case Study URL</span>
                <input
                  {...register("caseStudyUrl")}
                  placeholder="https://"
                  className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-cyan-200/70 focus:outline-none"
                />
                {errors.caseStudyUrl ? <p className="mt-1 text-xs text-rose-100">{errors.caseStudyUrl.message}</p> : null}
              </label>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Omslagsbild</span>
                <input
                  ref={coverFileRef}
                  type="file"
                  accept="image/*"
                  className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-sm text-white file:mr-3 file:rounded-full file:border-0 file:bg-white/20 file:px-3 file:py-1 file:text-xs file:font-semibold file:text-white"
                />
                {selectedProject?.visuals.coverPath ? (
                  <p className="mt-1 text-xs text-white/55">Nuvarande: {selectedProject.visuals.coverPath}</p>
                ) : null}
              </label>

              <label className="block">
                <span className="mb-2 block text-xs uppercase tracking-[0.2em] text-white/60">Galleri (flera filer)</span>
                <input
                  ref={galleryFileRef}
                  type="file"
                  accept="image/*"
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {isEditMode ? (
                <button
                  type="button"
                  onClick={() => setProjectForEditing(null)}
                  className="rounded-xl border border-white/20 bg-white/10 px-5 py-2 text-sm font-semibold text-white"
                >
                  Avbryt redigering
                </button>
              ) : null}
